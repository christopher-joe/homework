...
const devicesId = "646522489" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "4DsuCJwlaJLKxFtp6m95g9We16c=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=Temperature,Humidity,number&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          if (response.data.datastreams.length === 0) {
            reject("当前设备无数据, 请先检查硬件")
          }

          //程序可以运行到这里说明请求成功, 将Promise置为resolve状态
          resolve({
            number: response.data.datastreams[1].datapoints.reverse()
          })
        },
        fail: (err) => {
          reject(err)
